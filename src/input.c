...
/*   Updated: 2023/01/11 11:06:07 by pschwarz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_map	*parse_map(char *path)
{
	...
	res->map_coordinates = malloc((res->height * res->width * sizeof(t_coordinates *))
			+ sizeof(void *));
	if (!res)
		return (free(mapstring), NULL);
	populate_map(mapstring, res->map_coordinates);
	free(mapstring);
	return (res);
}
